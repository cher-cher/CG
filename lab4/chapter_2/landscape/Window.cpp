...
namespace
{
	const char LANDSCAPE_TEX_PATH[] = "res/galaxy/tex.bmp";
	...
	const unsigned SPHERE_PRECISION = 10;
	const float CAMERA_INITIAL_ROTATION = 0;
	const float CAMERA_INITIAL_DISTANCE = 300.f;
...
void CWindow::OnWindowInit(const glm::ivec2 &size)
{
	...
	m_pLandscapeTexture = loader.Load(LANDSCAPE_TEX_PATH);
...
void CWindow::OnDrawWindow(const glm::ivec2 &size)
{
	...
	m_material.Setup();
	m_pSkybox->Draw();
	m_pLandscapeTexture->DoWhileBinded([&] {
		m_pLandscape->Draw();
	});
}
